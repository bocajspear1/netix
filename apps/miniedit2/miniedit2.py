...
class MiniEdit( Frame ):
    ...
    def __init__( self, parent=None, cheight=600, cwidth=1000 ):
        ...
        self.nodePrefixes = { 'LegacyRouter': 'r', 'LegacySwitch': 's', 'SDNSwitch': 's', 'Host': 'h' , 'Controller': 'c'}
        self.widgetToItem = {}
        self.itemToWidget = {}
        ...
        self.hostPopup.add_command(label='Delete', font=self.font, command=self.deleteSelection )
        ...
        self.switchPopup.add_command(label='Delete', font=self.font, command=self.deleteSelection )

        self.switchLegacyPopup = Menu(self.top, tearoff=0)
        self.switchLegacyPopup.add_command(label='Delete', font=self.font, command=self.deleteSelection )
        ...
        self.linkPopup.add_command(label='Delete', font=self.font, command=self.deleteSelection )
        ...
        self.controllerPopup.add_command(label='Delete', font=self.font, command=self.deleteSelection )
    ...
    def _closePopupMenu(self, event):
        event.widget.unpost()

    def _openPopupMenu(self, menu, event):
        # If we have a tool open, revert to pointer
        self.activate('Select')
        menu.bind("<FocusOut>", self._closePopupMenu)
    ...
    def createMenubar( self ):
        ...
        fileMenu.add_command( label="New Topology", font=font, command=self.newTopology )
        fileMenu.add_command( label="Open Topology", font=font, command=self.loadTopology )
        fileMenu.add_command( label="Save Topology", font=font, command=self.saveTopology )
        ...
        topologyMenu = Menu( mbar, tearoff=False )
        mbar.add_cascade( label="Topology", font=font, menu=topologyMenu )
        topologyMenu.add_command( label="Run", font=font, command=self.doRun )
    ...
    def doRun( self ):
        ...
        self.canvas.configure(bg='#E4E4E4')
    ...
    def loadTopology( self ):
        ...
        if 'application' in loadedTopology:
            if self.appPrefs is not None:
                self.appPrefs.update(loadedTopology['application'])
            else:
                self.appPrefs = loadedTopology['application']
        ...
        for switch in switches:
            ...
            if switch['opts']['switchType'] == "legacyRouter":
                ...
                self.addNode('SDNSwitch', nodeNum, float(x), float(y), name=hostname)
                icon = self.findWidgetByName(hostname)
    ...
    def newTopology( self ):
        "New command."
        all_keys = list(self.widgetToItem.keys())
        for i in range(len(all_keys)):
            widget_key = all_keys[i]
            self.deleteItem( self.widgetToItem[ widget_key ] )
        self.hostCount = 0
        self.switchCount = 0
        self.controllerCount = 0
        self.links = {}
    ...
    def saveTopology( self ):
        ...
        if len(fileName ) > 0:
            ...
            for widget in self.widgetToItem:
                ...
                if 'SDNSwitch' in tags or 'LegacySwitch' in tags or 'LegacyRouter' in tags:
    ...
    def deleteSelection( self, _event=None ):
    ...
    def clickSDNSwitch( self, event ):
        "Add a new switch to our canvas."
        self.newNode( 'SDNSwitch', event )
    ...
    def listBridge( self, _ignore=None ):
        ...
        if name not in self.net.nameToNode:
            return
        if 'SDNSwitch' in tags or 'LegacySwitch' in tags:
    ...
    def do_linkPopup(self, event):
        ...
        if self.net is None:
            self._openPopupMenu(self.linkPopup, event)
        else:
            self._openPopupMenu(self.linkRunPopup, event)

    def do_controllerPopup(self, event):
        # display the popup menu
        if self.net is None:
            self._openPopupMenu(self.controllerPopup, event)

    def do_legacyRouterPopup(self, event):
        # display the popup menu
        if self.net is not None:
            self._openPopupMenu(self.legacyRouterRunPopup, event)
        else:
            self._openPopupMenu(self.legacyRouterPopup, event)

    def do_hostPopup(self, event):
        # display the popup menu
        if self.net is None:
            self._openPopupMenu(self.hostPopup, event)
        else:
            self._openPopupMenu(self.hostRunPopup, event)

    def do_legacySwitchPopup(self, event):
        # display the popup menu
        if self.net is None:
            self._openPopupMenu(self.switchLegacyPopup, event)
        else:
            self._openPopupMenu(self.switchRunPopup, event)
        

    def do_switchPopup(self, event):
        # display the popup menu
        if self.net is None:
            self._openPopupMenu(self.switchPopup, event)
        else:
            self._openPopupMenu(self.switchRunPopup, event)
    ...
    def importTopo( self ):
        ...
        for switch in importNet.switches:
            ...
            self.addNode('SDNSwitch', self.switchCount,
                 float(x), float(currentY), name=name)
            icon = self.findWidgetByName(name)
            icon.bind('<Button-3>', self.do_switchPopup )
